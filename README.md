...
启动 HTTP 控制面服务，并在启动时应用或导出当前 WireGuard 配置。

| 参数 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- |
| `--config` | `./config.json` | 否 | 本节点 JSON 配置路径 |
| `--device` | `wg0` | 否 | WireGuard 设备名 |
| `--listen` | `:8080` | 否 | HTTP API 监听地址 |
| `--backend` | `auto` | 否 | Backend 名称：`auto`、`wgconfig-file`、`linux-native`、`windows-service`、`windows-nt`、`darwin-kit`、`android-tunnel` |
| `--output` | 空 | 否 | 非 native backend 导出的 WireGuard `.conf` 路径；为空则不导出 |

### `connect`

连接远端 `/connect`，保存分配地址和远端 Peer，并应用或导出本地配置。

| 参数 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- |
| `--config` | `./config.json` | 否 | 本节点 JSON 配置路径 |
| `--device` | `wg0` | 否 | WireGuard 设备名 |
| `--url` | 空 | 是 | 远端 `/connect` URL |
| `--backend` | `auto` | 否 | Backend 名称 |
| `--output` | 空 | 否 | 非 native backend 导出的 WireGuard `.conf` 路径；为空则不导出 |

### `peers`

查询远端 `/peers` 并原样输出 JSON。

| 参数 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- |
| `--url` | 空 | 是 | 远端 `/peers` URL |

### `disconnect`

请求远端 `/disconnect`，删除本地 Peer，并应用或导出本地配置。

| 参数 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- |
| `--config` | `./config.json` | 否 | 本节点 JSON 配置路径 |
| `--device` | `wg0` | 否 | WireGuard 设备名 |
| `--url` | 空 | 是 | 远端 `/disconnect` URL |
| `--public-key` | 空 | 是 | 要断开的 Peer 公钥 |
| `--backend` | `auto` | 否 | Backend 名称 |
| `--output` | 空 | 否 | 非 native backend 导出的 WireGuard `.conf` 路径；为空则不导出 |

### `export`

把本地 `NodeConfig` 导出为标准 WireGuard `.conf`。

| 参数 | 默认值 | 必填 | 说明 |
| --- | --- | --- | --- |
| `--config` | `./config.json` | 否 | 本节点 JSON 配置路径 |
| `--output` | 空 | 是 | 导出的 WireGuard `.conf` 路径 |
...
- `darwin-kit` / `android-tunnel`：平台 native 接口已预留，当前 native apply 返回未实现
...
```powershell
go build -o build/easywired.exe ./cmd
```

启动一个可分配地址的节点：

```powershell
.\build\easywired.exe serve --config examples/nodeA.json --device wg0 --listen :8080 --backend auto --output nodeA-wg0.conf
```

新节点连接到 nodeA：

```powershell
.\build\easywired.exe connect --config examples/nodeB.json --device wg0 --url http://nodeA:8080/connect --backend auto --output nodeB-wg0.conf
```

查询远端 Peer：

```powershell
.\build\easywired.exe peers --url http://nodeA:8080/peers
```

断开 Peer：

```powershell
.\build\easywired.exe disconnect --config examples/nodeB.json --device wg0 --url http://nodeA:8080/disconnect --public-key <peer-public-key> --backend auto --output nodeB-wg0.conf
```

只导出 WireGuard 配置：

```powershell
.\build\easywired.exe export --config examples/nodeB.json --output nodeB-wg0.conf
```

Docker Compose 运行 server：

```powershell
docker compose up easywired-server
```

默认会使用 `examples/nodeA.json` 启动 `serve`，监听宿主机 `8080` 端口，并把生成的 `nodeA-wg0.conf` 写到 `build/` 目录。
